  ...
  return (
    <div className="w-full p-5">
      ...
      <div className="flex justify-between items-center w-full mb-3">
        <h1 className="font-semibold text-md">History</h1>
        <Link href="/view-all">
          <h1 className="font-medium text-yellow-400 text-md">View all</h1>
        </Link>
      </div>
            ...
            {data?.data?.length <= 0 ? (
              ...
              data?.data?.map((items: responseHistoryPayment) => (
                <CardHistory
                  ...
                  status={items.statusPayment === "PAID" ? "paid" :(items.statusPayment === "PENDING" ? "pending" : "failed")}
                />
              ))
            )}
          </div>
        ...
        {activeTab === 'parking' && (
          <div className="overflow-y-auto overflow-x-hidden aspect-[4/3] space-y-2">
            {(parkingHistory?.data?.length <= 0) ? (
              ...
              parkingHistory?.data?.map((items: responseHistoryParking, index: number) => (
                <CardHistory
                  key={index}
                  type="parking"
                  date={items.time}
                  ...
                  status={items.type === "Masuk Area Parkir" ? "masuk" : "keluar"}
