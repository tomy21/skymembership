...
export default function TableRecon({ bankName }: propsBank) {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      const res = await fetch("/api/reconsiliasi/uploadMutasi", {
        method: "POST",
        body: formData,
        credentials: "include",
      });
                      ...
                      <TableRow key={index}>
                        ...
                        <TableCell className="text-theme-xs px-5 py-3 text-center font-medium whitespace-nowrap text-gray-500 dark:text-gray-400">
                          <Button
                            onClick={() =>
                              router.push(
                                `/admin/reconsiliasi/${bankName === "BAYARIND_BCA_VIRTUAL_ACCOUNT" ? "bayarind" : "nobu"}/${encodeURIComponent(items.month)}`,
                              )
                            }
                            variant="outline"
