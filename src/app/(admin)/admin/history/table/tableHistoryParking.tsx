...
export default function TableHistoryParking() {
  const [search, setSearch] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [selectedLimit, setSelectedLimit] = useState<string>("10");
  const [totalPages, setTotalPages] = useState(1);
  const [mounted, setMounted] = useState(false);
  const [dataHistory, setDataHistory] = useState<HistoryParkingData[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const response = await axios.get("/api/history-parking", {
          params: {
            page: currentPage,
            limit: selectedLimit,
            search,
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL_USERS}/v01/cms/api/export-data-post?${params.toString()}`,
        {
          method: "GET",
          credentials: "include",
          headers: {
                  ...
                  {isLoading ? (
                    ...
                    dataHistory.map((items, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell className="text-theme-sm px-5 py-3 text-start font-medium whitespace-nowrap text-gray-500 dark:text-gray-400">
                          <div className="flex flex-col items-start justify-start">
                            <h1 className="font-semibold whitespace-nowrap text-gray-500 dark:text-gray-400">
                              {items.userHistoryPost.fullname}
                            </h1>
                            <h1 className="font-medium text-gray-300 dark:text-gray-200">
                              {items.userHistoryPost?.email}
                            </h1>
                          </div>
                        </TableCell>
                        <TableCell className="text-theme-sm px-5 py-3 text-start font-medium whitespace-nowrap text-gray-500 dark:text-gray-400">
                        ...
                        <TableCell className="text-theme-sm px-5 py-3 text-start font-medium whitespace-nowrap text-gray-500 dark:text-gray-400">
                          {Number(items.tariff).toLocaleString("id-ID")}
                        </TableCell>
                        <TableCell className="text-theme-sm px-5 py-3 text-start font-medium whitespace-nowrap text-gray-500 dark:text-gray-400">
                          {Number(items.balance_before).toLocaleString("id-ID")}
                        </TableCell>
                        <TableCell className="text-theme-sm px-5 py-3 text-start font-medium whitespace-nowrap text-gray-500 dark:text-gray-400">
                          {Number(items.balance_after).toLocaleString("id-ID")}
                        </TableCell>
                        <TableCell>
                          <Badge
                            size="sm"
                            color={
                              items.status_member === "MEMBER"
                                ? "success"
                                : "error"
                            }
                          >
                        ...
                        <TableCell>
                          <Badge
                            size="sm"
                            color={items.is_close === 1 ? "success" : "error"}
                          >
                            {items.is_close === 1 ? "Out" : "In Area"}
                          </Badge>
                        </TableCell>
                      </TableRow>
                    ))
