...
// Create context
const PaymentContext = createContext<TransactionContextType | undefined>(undefined);
...
export const PaymentProvider: React.FC<TransactionProviderProps> = ({ children }) => {
    // Initialize state with null instead of undefined
    const [paymentData, setPaymentData] = useState<TransactionPayload | null>(null);
    const [admin_fee, setAdminFee] = useState<number>(0); // Optional, if you want to manage the fee

    return (
        <PaymentContext.Provider value={{ paymentData, setPaymentData, admin_fee, setAdminFee }}>
            {children}
